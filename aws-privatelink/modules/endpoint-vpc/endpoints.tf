locals {
  ...
  # service_region is null when the consumer VPC is in the Falcon cloud's
  # home region. Otherwise it's set to the home region so the endpoint
  # targets the service hosted there over the AWS backbone.
...
# for native deploys and the home region for cross-region deploys. Private
# DNS is not supported for these service types — resolution comes from the
# PHZ in route53.tf.
