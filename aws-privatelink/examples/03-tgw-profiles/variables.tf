...
variable "hub_profile" {
  description = "AWS CLI profile for the hub (networking) account. Hosts endpoint VPC, TGW, and R53 Profile."
  type        = string
}

variable "spoke_profile" {
  description = "AWS CLI profile for the spoke (workload) account. Owns a VPC that attaches to the hub TGW."
  type        = string
}

variable "spoke_account_id" {
  description = "12-digit account ID of the spoke account. Used as RAM principal for TGW and R53 Profile shares."
  type        = string
