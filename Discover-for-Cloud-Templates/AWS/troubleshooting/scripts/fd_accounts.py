...
def check_account():
    ...
    access_response = falcon.command(action="VerifyAWSAccountAccess", ids=id_list)
...
def register_account():
    ...
    if register_response["status_code"] == 201:
        ...
        print("Registration failed with response: {} {}".format(register_response["status_code"], register_response["body"]["errors"][0]["message"]))
...
def update_account():
    ...
    if update_response["status_code"] == 200:
        ...
        print("Update failed with response: {} {}".format(update_response["status_code"], update_response["body"]["errors"][0]["message"]))
...
def delete_account():
    ...
    if delete_response["status_code"] == 200:
        ...
        print("Delete failed with response: {} {}".format(delete_response["status_code"], delete_response["body"]["errors"][0]["message"]))
...
if __name__ == "__main__":
    ...
    if command.lower() in "check,update,register,delete":
        if command.lower() in "update,register":
            ...
            if (args.cloudtrail_bucket_owner_id is None or args.cloudtrail_bucket_region is None
                or args.local_account is None or args.external_id is None or args.iam_role_arn is None):
